...
GLfloat bottom = 0.0;
GLfloat top    = 1.0;
GLint game_width  = 300;
GLint game_height = 300;
	...
	glBegin(GL_QUADS);
		for (GLint x = 0; x < game_width; ++x) {
			for (GLint y = 0; y < game_height; ++y){
				...
				if(tem_formiga_viva(map, x, y)){
					Ant *a = get_alive_ants_grid(map, x, y);
...
int main(int argc, char **argv){
	...
	map->n_alive_ants = 1000;
	map->n_dead_ants = 25000;
...
void create_ants(Map *map){
	...
	for (int i = 0; i < map->n_alive_ants; ++i){
		...
		map->alive_ants[i].x_old = coor[pos][0];
		map->alive_ants[i].y_old = coor[pos][1];
	...
	for (int i = 0; i < map->n_dead_ants; ++i){
		...
		map->dead_ants[i].x_old = coor[pos][0];
		map->dead_ants[i].y_old = coor[pos][1];
...
void move_ant(Map *map, Ant *ant){
	...
	for (int i = 0; i < 8; ++i){
		if(is_valid_coordinates(map, neighbor_coor[direction][0], neighbor_coor[direction][1])){
			if(!tem_formiga_viva(map, neighbor_coor[direction][0], neighbor_coor[direction][1]) && neighbor_coor[direction][0] != ant->x_old && neighbor_coor[direction][1] != ant->y_old){
...
void map_iterate(Map *map){
	#pragma omp parallel for num_threads(4) //lento
	for (int i = 0; i < map->n_alive_ants; ++i){

		int n_dead_neighbor_ants = counts_dead_neighbor_ants(map, map->alive_ants[i], 3);

		if(map->alive_ants[i].state == CARRING){
			if(!tem_formiga_morta(map, map->alive_ants[i].x, map->alive_ants[i].y)){

				int prob_jogar = pow((n_dead_neighbor_ants / 48.0), 2) * 100;
			...
			if(tem_formiga_morta(map, map->alive_ants[i].x, map->alive_ants[i].y)){			
				...
				int prob_pegar = pow((48.0 - n_dead_neighbor_ants) / 48.0, 2) * 100;
